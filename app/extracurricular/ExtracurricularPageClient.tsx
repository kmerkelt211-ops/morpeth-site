...
import { pickRemoteMediaUrl } from "../../lib/mediaPolicy";
...
type VideoCard = {
  ...
  videoUrl?: string;
  videoFileUrl?: string;
  hoverLoopFileUrl?: string;
  hoverLoopFileSize?: number;
...
export type PartialExtracurricularContent = Partial<{
...
function resolveHoverPreviewSource(card: {
  hoverLoopFileUrl?: string;
  hoverLoopFileSize?: number;
}): string {
  return (
    pickRemoteMediaUrl({
      url: card.hoverLoopFileUrl,
      size: card.hoverLoopFileSize,
    }) || ""
  );
}

function ClubVideoCard({
  card,
  onExpand,
}: {
  card: VideoCard;
  onExpand: (card: VideoCard, src: string, isYoutube: boolean) => void;
}) {
  const videoSrc = resolveVideoSource(card);
  const cardIsYoutube = videoSrc ? isYoutubeUrl(videoSrc) : false;
  const hoverPreviewSrc = resolveHoverPreviewSource(card);
  ...
  return (
    <button
      type="button"
      onClick={() => {
        onExpand(card, videoSrc, cardIsYoutube);
      }}
    ...
    >
      <div className="relative aspect-video w-full overflow-hidden rounded-2xl bg-black">
        {hoverPreviewSrc && isPreviewActive ? (
          <video
            key={hoverPreviewSrc}
          ...
          >
            <source src={hoverPreviewSrc} type="video/mp4" />
          </video>
        ) : videoSrc ? (
          cardIsYoutube ? (
            <>
...
type ExtracurricularPageClientProps = {
  initialContent?: PartialExtracurricularContent | null;
};

export default function ExtracurricularPageClient({
  initialContent,
}: ExtracurricularPageClientProps) {
  const [content] = useState<ExtracurricularContent>(() =>
    mergeContent(initialContent)
  );
            ...
            <div className="flex snap-x snap-mandatory gap-4 px-4 scroll-px-4">
              {content.clubVideos.cards.map((card) => (
                <ClubVideoCard
                  ...
                  card={card}
                  onExpand={(expandedCard, src, isYoutube) => {
                    setExpandedClub({ card: expandedCard, src, isYoutube });
                  }}
                />
              ))}
            </div>
