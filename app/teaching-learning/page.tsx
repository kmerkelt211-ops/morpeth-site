...
const DEFAULT_SUBJECTS: SubjectCard[] = [
  {
    id: "art-photography",
...
type PartialTeachingContent = Partial<{
...
export default function TeachingLearningPage() {
  const [content, setContent] = useState<TeachingLearningContent>(DEFAULT_CONTENT);
  ...
  useEffect(() => {
    ...
    async function fetchContent() {
      try {
        const response = await fetch("/api/teaching-learning", { cache: "no-store" });
        if (!response.ok) return;
        const data = (await response.json()) as PartialTeachingContent;
        if (mounted) {
          setContent(mergeTeachingContent(data));
        }
      } catch {
